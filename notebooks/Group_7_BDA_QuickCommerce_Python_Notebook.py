...
# MAGIC | Phase 3 | Machine Learning | MLlib — K-Means, GBT classification, GBT regression, FP-Growth |
# MAGIC
...
# MAGIC %md
# MAGIC ## Phase 1.5 — Curated Table Layer (Hive-style Metastore)
# MAGIC The views registered above are session-scoped. A production Hadoop stack persists
# MAGIC curated data as **managed tables in a metastore** — historically Hive, here Unity
# MAGIC Catalog — so that schema is enforced on write, tables survive the session, and any
# MAGIC SQL client can query them without knowing the underlying file layout.
# MAGIC
# MAGIC This is the data governance layer: schema enforcement, discoverability and a single
# MAGIC definition of each table that downstream consumers share.
...
# MAGIC %md
# MAGIC ### Phase 2.7 — Checkpoint to Managed Tables
# MAGIC Everything above is expensive to recompute. Free-tier serverless compute can
# MAGIC reclaim a session during a long run, which destroys all Python state and forces
# MAGIC a restart from cell one.
# MAGIC
# MAGIC We therefore persist the two expensive Phase 2 outputs as managed tables. If the
# MAGIC session is lost, run the **Setup** cell at the top followed by the **Resume** cell
# MAGIC below, and continue from Phase 3 without recomputing anything.
...
# MAGIC %md
# MAGIC ### RESUME CELL — run this only after a session restart
# MAGIC Set `RESUMING = True` below, run the Setup cell at the top of this notebook,
# MAGIC then run this cell. It restores the Phase 2 outputs from the checkpoint tables
# MAGIC so Phase 3 can proceed. Leave `RESUMING = False` during a normal run.
...
# MAGIC %md
# MAGIC ### 3B.1 — Candidate model comparison
# MAGIC Three classifier families are compared on identical features and an identical split.
# MAGIC Selecting a model without comparing alternatives is an assertion, not a justification.
...
# MAGIC %md
# MAGIC ### 3B.2 — Hyperparameter tuning
# MAGIC Default hyperparameters are a starting point, not a result. We sweep tree depth
# MAGIC and boosting iterations for the selected model family.
...
# MAGIC %md
# MAGIC ### 3B.3 — Final model
# MAGIC The winning configuration is refit on the **full** training set, not the
# MAGIC selection subsample, and evaluated on the held-out test set.
...
# MAGIC %md
# MAGIC ## Phase 3C — Reorder Timing (Regression)
# MAGIC Classification answers **what** a customer will reorder. It does not answer **when**.
# MAGIC For a dark store, timing is the operational half of the problem: stock arriving a week
# MAGIC early ties up shelf space, stock arriving a day late loses the order.
# MAGIC
# MAGIC Target: days until the customer's next order, predicted from their historical
# MAGIC ordering behaviour.
...
# MAGIC %md
# MAGIC ## Phase 3D — Market Basket Optimization (FP-Growth)
